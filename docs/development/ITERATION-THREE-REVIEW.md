...
| I03-R26: safe-root validation rejected Windows short-path aliases | Hosted Windows setup fixtures stopped before planning because `RUNNER~1` and its canonical spelling differed. | Inspect every root ancestor for redirects, then canonicalize equivalent directories and compare filesystem identity; preserve root/ancestor junction rejection and test actual aliases. |
| I03-R27: maker tests copied the consumer's live business registry | A plugin author adding Bookmarks broke the template's own qualification fixtures. | Use fixture-owned schemas/registry and only shared foundation inputs; rerun the maker checks from an actually extended consumer with removed examples. |
| I03-R28: native modal focus ran before the host's own initial-focus step | Obsidian selected Cancel after the service selected Confirm. | Apply owned focus after the public Modal.open lifecycle, with a host double that reproduces actual ordering; retain the native focus assertion. |
| I03-R29: two guarded Nuxt modules shared one replacement plugin object | Normal component mounts emitted duplicate-plugin warnings. | Give each unchanged hash-guarded replacement its own module identity and fail real-component tests on unexpected warnings. |
| I03-R30: host attachment failed before Vue had completed mounting | Public unmount could not clean an incompletely mounted app, leaving subscriptions and render context behind. | Fully mount into an owned detached surface with owned teleports before host attachment; guard resource acquisition and teardown, and test both pre-mount and attachment failures. This does not claim recovery from arbitrary internal Vue renderer corruption. |
| I03-R31: each view installed the runtime's shared i18n plugin | Vue-i18n's app-unmount wrapper disposed translation state still owned by other views and native services. | Give views private composers driven by canonical preferences, dispose the runtime composer only with the runtime, and verify locale changes after sibling close and failed attachment. |
