...
The title stem is generated by a tested safe-path policy. The title itself is preserved as content. A title is never interpreted as a destination path. The ID distinguishes separate Tasks with equal titles. Folder settings are validated and confined to the allowed vault location.
...
Its illustrative path is `Tasks/prepare-release-checklist--task-example-001.md`. The fixture ID is not a prescribed production ID format. The serializer's qualified stable output controls exact quoting/formatting; tests additionally verify property meanings through host parsing.
