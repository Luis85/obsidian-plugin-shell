...
function designSnapshot(d){return designCopy({blueprint:d.blueprint,goal:d.goal,platform:d.platform,nodes:d.nodes,links:d.links,nextId:d.nextId,library:d.library,prds:d.prds,...(d.librarySchema?{librarySchema:d.librarySchema}:{}),...(d.canvas?{canvas:d.canvas}:{}),...(d.semantic?{semantic:d.semantic}:{}),...(d.dataSources?{dataSources:d.dataSources}:{}),...(d.designSystem?{designSystem:d.designSystem}:{}),...(d.storymaps?{storymaps:d.storymaps}:{}),...(d.detailDesigns?{detailDesigns:d.detailDesigns}:{})});}
function recordDesign(){const d=design();d.history.push(designSnapshot(d));d.history=d.history.slice(-DESIGN_LIMITS.history);d.future=[];cpBoundHistory(d);}
function designChanged(){const d=design();cpBoundHistory(d);d.revision++;designUi.plan=null;designUi.error='';save();}
function designHistory(direction){const d=design(),before=designFingerprint(d);const stack=direction==='undo'?d.history:d.future;if(!stack.length)return;const other=direction==='undo'?d.future:d.history;other.push(designSnapshot(d));if(other.length>20)other.shift();const retainedDetails=d.detailDesigns?dtCopy(d.detailDesigns):null;const detailCounter=d.detailDesigns?.nextId||1;const storymapCounter=d.storymaps?.nextId||1;const sourceCounter=d.dataSources?.nextId||1;const semanticCounter=d.semantic?.nextId||1;const nextId=d.nextId,snapshot=stack.pop();Object.assign(d,snapshot);if(snapshot.detailDesigns){d.detailDesigns.nextId=Math.max(detailCounter,d.detailDesigns.nextId);}else if(detailCounter>1)d.detailDesigns={...emptyDetailDesigns(),nextId:detailCounter};else delete d.detailDesigns;cpRetainRevisions(d.detailDesigns,retainedDetails);if(d.detailDesigns?.schema===2)d.schema=4;cpBoundHistory(d);d.storymaps=snapshot.storymaps||emptyStorymaps();d.storymaps.nextId=Math.max(storymapCounter,d.storymaps.nextId);if(!snapshot.dataSources)d.dataSources=emptyDataSources();d.dataSources.nextId=Math.max(sourceCounter,d.dataSources.nextId);if(!snapshot.semantic)d.semantic=emptySemantic();d.semantic.nextId=Math.max(semanticCounter,d.semantic.nextId);if(!snapshot.designSystem)delete d.designSystem;if(!snapshot.canvas)delete d.canvas;if(!snapshot.librarySchema)delete d.librarySchema;d.nextId=Math.max(nextId,d.nextId);if(before!==designFingerprint(d))designChanged();else save();render();notify((before!==designFingerprint(d)?'Outline ':'Map arrangement ')+direction+' complete. Generated source is never deleted.');}
...
function structuralDesign(value){
 if(!value||![1,2,3,4].includes(value.schema)||!Array.isArray(value.nodes)||value.nodes.length>DESIGN_LIMITS.nodes||!Array.isArray(value.links)||value.links.length>DESIGN_LIMITS.links)return false;
 ...
 return dtShape(value.detailDesigns)&&smShape(value.storymaps)&&validStyleGuide(value.designSystem)&&dataSourcesShape(value.dataSources)&&erShape(value.semantic)&&productShape(value)&&validCanvas(value.canvas)&&value.nodes.reduce((total,n)=>total+bricksOf(n).length,0)<=BRICK_LIMITS.total&&dataSourceIssues(value,false).length===0;
}
function designIssues(d){
 const issues=[...productIssues(d),...brickIssues(d),...semanticIssues(d),...dataSourceIssues(d),...smIssues(d),...dtIssues(d)];const add=(level,code,message,node=null)=>issues.push({level,code,message,node});
...
function importDesign(text){
 ...
 const allowed=new Set(['schema','blueprint','goal','platform','nodes','links','nextId','library','prds','librarySchema','kind','executable','canvas','semantic','dataSources','designSystem','storymaps','detailDesigns']);if(Object.keys(input).some(k=>!allowed.has(k)))throw Error('Unknown blueprint properties are not accepted.');const nextId=importCounter(input);const clean=ensureProductModel(designCopy(input));recordDesign();const d=design();Object.assign(d,{schema:4,detailDesigns:clean.detailDesigns?{...clean.detailDesigns,nextId:Math.max(d.detailDesigns?.nextId||1,clean.detailDesigns.nextId)}:undefined,storymaps:{...(clean.storymaps||emptyStorymaps()),nextId:Math.max(d.storymaps?.nextId||1,clean.storymaps?.nextId||1)},blueprint:clean.blueprint,goal:clean.goal,platform:clean.platform,nodes:clean.nodes,links:clean.links,library:clean.library,librarySchema:clean.librarySchema,prds:clean.prds,semantic:{...(clean.semantic||emptySemantic()),nextId:Math.max(d.semantic?.nextId||1,clean.semantic?.nextId||1)},dataSources:{...(clean.dataSources||emptyDataSources()),nextId:Math.max(d.dataSources?.nextId||1,clean.dataSources?.nextId||1)},designSystem:clean.designSystem,canvas:clean.canvas||emptyCanvas(),nextId});designChanged();designUi.selected=d.nodes[0]?.id;return d;
}
