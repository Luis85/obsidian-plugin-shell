...
function designSnapshot(d){return designCopy({blueprint:d.blueprint,goal:d.goal,platform:d.platform,nodes:d.nodes,links:d.links,nextId:d.nextId,library:d.library,prds:d.prds,...(d.librarySchema?{librarySchema:d.librarySchema}:{}),...(d.canvas?{canvas:d.canvas}:{}),...(d.semantic?{semantic:d.semantic}:{}),...(d.dataSources?{dataSources:d.dataSources}:{}),...(d.designSystem?{designSystem:d.designSystem}:{})});}
...
function structuralDesign(value){
 ...
 return validStyleGuide(value.designSystem)&&dataSourcesShape(value.dataSources)&&erShape(value.semantic)&&productShape(value)&&validCanvas(value.canvas)&&value.nodes.reduce((total,n)=>total+bricksOf(n).length,0)<=BRICK_LIMITS.total&&dataSourceIssues(value,false).length===0;
}
function designIssues(d){
 const issues=[...productIssues(d),...brickIssues(d),...semanticIssues(d),...dataSourceIssues(d)];const add=(level,code,message,node=null)=>issues.push({level,code,message,node});
