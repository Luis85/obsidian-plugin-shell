...
function design(){const p=project();if(p){if(!p.design)p.design=createDesign('blank');return ensureProductModel(p.design);}if(!state.designDraft)state.designDraft=createDesign('workspace');return ensureProductModel(state.designDraft);}
function designOwner(){return project()?.id||'unattached-draft';}
function designSnapshot(d){return designCopy({blueprint:d.blueprint,goal:d.goal,platform:d.platform,nodes:d.nodes,links:d.links,nextId:d.nextId,library:d.library,prds:d.prds,...(d.librarySchema?{librarySchema:d.librarySchema}:{}),...(d.canvas?{canvas:d.canvas}:{})});}
...
function designHistory(direction){const d=design(),before=designFingerprint(d);const stack=direction==='undo'?d.history:d.future;if(!stack.length)return;const other=direction==='undo'?d.future:d.history;other.push(designSnapshot(d));if(other.length>20)other.shift();const nextId=d.nextId,snapshot=stack.pop();Object.assign(d,snapshot);if(!snapshot.canvas)delete d.canvas;if(!snapshot.librarySchema)delete d.librarySchema;d.nextId=Math.max(nextId,d.nextId);if(before!==designFingerprint(d))designChanged();else save();render();notify((before!==designFingerprint(d)?'Outline ':'Map arrangement ')+direction+' complete. Generated source is never deleted.');}
function selectedNode(){const d=design();return d.nodes.find(n=>n.id===designUi.selected)||null;}
...
function designIssues(d){
 const issues=[...productIssues(d),...brickIssues(d)];const add=(level,code,message,node=null)=>issues.push({level,code,message,node});
...
function designFingerprint(d){const snapshot=designSnapshot(d);delete snapshot.canvas;return JSON.stringify(snapshot);}
