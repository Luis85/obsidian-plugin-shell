...
function companionExampleProject() {
  ...
  const screens = [
    ...
    ['page-editor', 'Page editor', 'canvas', 'Compose responsive regions, token styles, reusable instances and named slot content using Vue Flow. Review layouts and fixture states, play declared UI effects and export implementation contracts.'],
    ['component-editor', 'Component editor', 'canvas', 'Design reusable internals and public property/slot contracts. Publish immutable revisions, compare variants and review direct or transitive consumer impact before upgrading an instance.'],
    ...
    ['design-system', 'Design System', 'list-detail', 'Edit fonts, typography, spacing, sizes, radii, colors and guidance; export Markdown, HTML or scoped Nuxt UI CSS.'],
...
function companionExampleRequirements(d, surfaces) {
  const groups = [
    ['Product & workspace', 'A plugin idea, its requirements and its implementation context are otherwise scattered across tools.', [
      ...
      ['Use reusable components', 'components', 'Place pinned component revisions and variants with local overrides and named slot content. Preview variants, inspect transitive usage, and review contract differences and slot remapping before applying upgrades.'],
    ...
    ['Data & visual contracts', 'Developers need explicit data and visual contracts before implementation can be safely generated.', [
      ...
      ['Author the design system', 'design-system', 'Persist font declarations, typography, sizes, spacing, radii, paired colors, usage guidance and explicit Nuxt UI token bindings.'],
      ['Export a readable design guide', 'design-system', 'Markdown and offline HTML contain all saved declarations and disclose unevaluated contrast pairs. A scoped stylesheet export and the JSON-driven shell generator compile the same saved Nuxt UI token bindings.'],
    ]],
    ['Project JSON & shell handoff', 'A developer needs one complete portable definition that both the design tool and shell can read.', [
      ...
      ['Keep generation a separate stage', 'prepare', 'The independent companion:scaffold / shell generate compiler reviews an ownership-safe apply plan, including compiled design-system styles. The read-only version-one handoff remains write-free.'],
    ]],
