...
function workflowFacts(){
 ...
 const issues=designIssues(d),blockers=issues.filter(x=>x.level==='error');
 ...
 return {d,p,active,requirements,issues,blockers,used,mapped,hasBrief:active.some(x=>x.problem.trim())||Boolean(d.goal.trim()),
...
function workflowIssueDestination(issue){
 ...
 if(/component|binding|version|slot/.test(issue.code||''))return {action:'nav',value:'components'};
 if(/requirement|prd/.test(issue.code||''))return {action:'nav',value:'prds'};
