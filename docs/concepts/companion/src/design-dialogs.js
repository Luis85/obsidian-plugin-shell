...
function designReplaceDialog(){const b=BLUEPRINTS.find(b=>b.id===modalData);return dialogBody('Use '+b.name+'?',`${layoutThumbnail(b.layout,true)}<p>${b.description}</p>${callout('This replaces the editable sitemap only. PRDs and components are preserved. Existing requirement links to replaced surfaces become explicit blockers; no source files are deleted. Undo restores the previous outline.','warn')}`,button('Cancel','close','','ghost')+button('Use blueprint','blueprint-apply',b.id,'primary','check'));}
function designRemoveDialog(){const n=design().nodes.find(n=>n.id===modalData);if(!n)return dialogBody('Surface not found','It may already have been removed.');const nodes=nodeDescendants(design(),n.id);return dialogBody('Remove '+n.label+'?',`<p>This removes ${nodes.size} surface(s) and ${design().links.filter(e=>nodes.has(e.from)||nodes.has(e.to)).length} related user-flow connection(s) from the outline.</p><p class="small muted">${allRequirements().filter(r=>r.nodes.some(id=>nodes.has(id))).length} requirement reference(s) are retained and will need remapping. Shared component definitions are kept.</p>${callout('Previously scaffolded files are retained. This is not a code deletion or migration. You can undo the outline change.','warn')}`,button('Cancel','close','','ghost')+button('Remove from outline','design-remove-confirm',n.id,'danger'));}
function designTransferDialog(){return dialogBody('Portable plugin blueprint',`<p>Export the sitemap, layouts, patterns and product intent as data. No paths, execution trust, credentials or test results are included. Import validates schema and structure before replacing the outline.</p><label for="blueprint-json">Blueprint JSON</label><textarea id="blueprint-json" class="mono transfer-area" spellcheck="false">${esc(JSON.stringify(portableDesign(),null,2))}</textarea><p id="design-error" class="error" role="alert">${esc(designUi.error)}</p>`,button('Close','close','','ghost')+button('Import reviewed JSON','design-import','','','upload')+button('Download blueprint','design-export','','primary','download'));}
function startNodeForm(kind='view',editId=null){
 ...
 designUi.form=n?{...designCopy(n),intent:n.intent||'',goals:n.goals||[],editing:true,owner:designOwner()}:{components:[],id:'node-'+d.nextId,label:kind==='view'?'New view':kind==='page'?'New screen':'New surface',slug:kind==='view'?'new-view':kind==='page'?'new-screen':'new-surface',kind,layout:'single',placement:'tab',parent,nav:kind==='page',command:kind==='view',ribbon:false,entry:!d.nodes.some(n=>n.entry)&&kind==='view',instance:'reuse',patterns:[],goal:'',intent:'',goals:[],entity:'',editing:false,owner:designOwner()};
