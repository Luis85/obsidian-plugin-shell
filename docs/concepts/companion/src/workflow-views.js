...
function workflowOverview(){
 ...
 return heading(f.p?.name||'Your plugin workspace','The current vault is your project root. Design and source stay together.','<div class="row wrap">'+button('Project details','vault-init','','small','folder')+button('Export project JSON','project-export','','small','download')+button('Import project JSON','project-import','','small','file')+button('Generate plugin shell','starter-generate','','small','terminal')+'</div>')+
 ...
 <section class="overview-detail"><h2>Review attention</h2>${workflowAttention(f)}
 <dl class="overview-facts"><dt>Brief</dt><dd>${f.active.length} active PRD(s)</dd><dt>Traceability</dt><dd>${f.requirements.length?f.mapped+' / '+f.requirements.length+' requirements mapped':'No active requirements yet'}</dd><dt>Frontend</dt><dd>${f.d.nodes.filter(x=>x.kind==='view').length} native views · ${f.d.nodes.filter(x=>x.kind==='page').length} internal screens</dd><dt>Semantic layer</dt><dd>${f.d.semantic?.entities.length||0} entities · ${f.d.semantic?.relationships.length||0} relationships</dd><dt>Data Sources</dt><dd>${f.d.dataSources?.sources.length||0} sources · ${f.d.dataSources?.flows.length||0} data flows</dd><dt>Components</dt><dd>${f.used} bindings · ${f.d.library.length} contracts</dd><dt>Source plan</dt><dd>${f.generated?'Preview recorded at current design revision':'Review needed before scaffold preview'}</dd></dl>
...
// Show severity and destinations without turning optional design guidance into a gate.
function workflowAttention(f) {
 const issues=[...f.blockers,...f.warnings];
 if(!issues.length)return '<div class="attention-clear">'+icon('check')+'<div><strong>No structural findings</strong><p>Business behavior, usability and native compatibility still need verification.</p></div></div>';
 return `<p class="small muted">${f.blockers.length} blocking · ${f.warnings.length} advisory. Advisories do not prevent source review.</p>`+
 issues.slice(0,4).map(issue=>{const dest=workflowIssueDestination(issue);return `<div class="attention-item"><span class="attention-severity ${issue.level==='error'?'error':''}">${issue.level==='error'?'Blocking':'Advisory'}</span><p>${esc(issue.message)}</p>${button('Review',dest.action,dest.value,'small')}</div>`;}).join('')+
 (issues.length>4?`<p class="small muted">Showing 4 of ${issues.length} findings.</p>`:'')+
 button('All outline findings','workflow-checks','','ghost small');
}
