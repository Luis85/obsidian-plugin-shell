...
function smNodeComponent() {
  ...
  return defineComponent({ props: ['data', 'selected'], setup(p) { return () => {
    ...
    return h('article', { class: ['sm-card', 'sm-card-' + n.kind, { selected: p.selected }], 'data-sm-item': r.id, style: { width: n.width + 'px', height: n.height + 'px' } }, [
      h('div', { class: 'sm-card-main' }, [action(r.title, 'sm-select', r.id, { class: 'nodrag nopan sm-card-title', title: r.title, 'aria-label': n.kind + ': ' + r.title }),
        h('small', { class: 'sm-card-caption' }, n.kind === 'story' ? (r.surfaces.length ? r.surfaces.length + ' sitemap link' + (r.surfaces.length === 1 ? '' : 's') : r.ui === 'none' ? 'No UI required' : 'Not linked yet') : n.kind)]),
...
function smProjection(map) {
  const readOnly = map.status === 'archived' || !!state.activeRun;
  return smLayout(map).nodes.map(n => ({ id: n.id, type: 'storymap', position: { x: n.x, y: n.y },
    data: { ...n, readOnly }, dimensions: { width: n.width, height: n.height }, selected: n.id === smUi.item,
...
function smDropPreview(map, id, position) {
  const target = smDropTarget(map, id, position), item = smItem(map, id); let message = 'Drop outside the map cancels this move.';
  ...
  const layout = smLayout(map); let marker = document.getElementById('sm-insertion');
...
function smMount() {
  ...
  const Root = defineComponent({ setup() {
    ...
    api.onNodeDragStart(({ node }) => {
      if (!current()) return;
      try { smCanWrite(); smUi.cancelled = false; smUi.drag = { id: node.id, token: smToken(), layout: smLayout(smCurrentMap()) }; smSelect(node.id); }
      catch (error) { smUi.cancelled = true; smFail(error); render(); }
    });
    api.onNodeDrag(({ node }) => { if (current() && smUi.drag) { smDragGroup(smCurrentMap(), node); smDropPreview(smCurrentMap(), node.id, node.position); } });
    api.onNodeDragStop(({ node }) => {
      ...
      const target = smDropTarget(smCurrentMap(), node.id, node.position);
  ...
  smUi.resize = () => { if (!current()) return; if ((smUi.lastWidth > 1100) !== (innerWidth > 1100) && !smUi.item) smToggleDetails(innerWidth > 1100); smUi.lastWidth = innerWidth;
  ...
  smUi.resize(); nextTick(smUi.resize); window.addEventListener('resize', smUi.resize);
