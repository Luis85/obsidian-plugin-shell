...
function sgCommit(candidate){
 ...
 try{compileDesignSystem(candidate,project().id);}catch(error){return sgFail(error.message);}
...
function sgOpen(group,id=null){
 const s=styleGuide();if(!s||!['overview','frontend'].includes(group)&&!Object.hasOwn(SG_GROUPS,group))return sgFail('Create a design system first.');
 ...
 sgUi.form={group,id,record:designCopy(group==='overview'?{name:s.name,description:s.description,principles:s.principles}:group==='frontend'?(s.frontend||{schema:1,target:'nuxt-ui',colorPolicy:'host',bindings:{}}):saved||sgNew(group,s)),snapshot:sgIdentity(),owner:designOwner(),removal:false};sgUi.error='';showModal('style-guide-form');
}
...
function sgSave(){
 ...
 const s=designCopy(styleGuide());if(f.group==='overview')Object.assign(s,f.record);else if(f.group==='frontend')s.frontend=designCopy(f.record);else{const i=s[f.group].findIndex(r=>r.id===f.id);if(i<0)s[f.group].push(designCopy(f.record));else s[f.group][i]=designCopy(f.record);}
...
function handleStyleGuideAction(action,value=''){
 ...
 switch(action){
  case 'sg-section':if(['overview','frontend'].includes(value)||Object.hasOwn(SG_GROUPS,value)){sgUi.section=value;sgUi.error='';render();}break;
  ...
  case 'sg-export':if(['md','html','css'].includes(value))sgDownload(value);break;
 }
 return true;
}
function editStyleGuideField(el){
 if(sgEditFrontendField(el))return true;
