...
function smInput(label, key, value, max = 120, multiline = false) {
  return uiInput(label, 'sm-' + key, value, { multiline, rows: 3, extra: `maxlength="${max}"` });
}
function smSelectField(label, key, value, options) { return uiSelect(label, 'sm-' + key, options, value ?? ''); }
function smChecks(label, key, choices, selected, query = '') {
  ...
  return `<fieldset class="sm-reference-picker"><legend>${esc(label)}</legend><div class="sm-check-list">${rows.map(([id, title]) => `<label><input type="checkbox" data-field="sm-${key}" data-key="${esc(id)}" ${selectedIds.has(id) ? 'checked' : ''}><span>${esc(title)}</span></label>`).join('') || '<p class="muted small">No matching items. Existing selections are retained.</p>'}</div><small>${selectedIds.size} selected. Uncheck a missing target to remove its reference.</small></fieldset>`;
}
...
function smItemFields(f, map) {
  ...
  if (f.kind === 'story') {
    body += `<div class="sm-form-grid">${smSelectField('Step', 'stepId', r.stepId, map.steps.map(s => [s.id, s.title]))}${smSelectField('Release slice', 'releaseId', r.releaseId, [['', 'Unplanned'], ...map.releases.map(x => [x.id, x.title])])}</div>`;
  ...
  if (f.kind === 'story') body += smChecks('Requirements (optional)', 'requirementRef', design().prds.flatMap(p => p.requirements.map(q => [JSON.stringify([p.id, q.id]), p.title + ' / ' + q.title])), r.requirements);
  return body;
}
...
function storymapForm() {
  ...
  if (f.kind === 'link-prd') {
    title = 'Link storymaps'; body = '<p>Choose maps for this PRD. This changes links only; no stories are generated.</p>' + smChecks('Project storymaps', 'mapRef', smStore().maps.map(m => [m.id, m.title + (m.status === 'archived' ? ' · Archived' : '')]), f.record.maps);
  ...
  body += smFormError() + '<p class="small muted">Save updates the project. Diagram edits never generate source or change implementation evidence.</p>';
  return dialogBody(title, `<div class="sm-form">${body}</div>`, (f.id && !['move', 'link-prd'].includes(f.kind) ? button('Remove…', 'sm-delete', '', 'danger ghost', 'trash') : '') + button('Cancel', 'close', '', 'ghost') + button(f.kind === 'move' ? 'Move item' : 'Save', 'sm-save', '', 'primary'));
}
function editStorymapField(el) {
  ...
  if (prop === 'surfaceQuery') { f.surfaceQuery = value; redrawModal(); return true; }
  if (prop === 'surfaceRef') {
