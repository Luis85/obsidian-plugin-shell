...
function dtPreviewNodes(doc, parent, stack, stateName, budget, fills = {}, selected = null) {
  const assigned=new Set(doc.nodes.flatMap(n=>Object.values(n.slots || {}).flat()));
  return doc.nodes.filter(n => n.parentId === parent && n.visibleIn.includes(stateName) && (selected ? selected.includes(n.id) : parent !== null || !assigned.has(n.id))).map(n => {
    if (--budget.remaining < 0) return '';
    const label = esc(n.label), content = esc(n.text || n.label);
    if (n.kind === 'region') return `<section class="dt-wire-region layout-${n.layout}"><span class="dt-wire-label">${label}</span>${dtPreviewNodes(doc, n.id, stack, stateName, budget, fills)}</section>`;
    if (n.kind === 'text') return `<p class="dt-wire-text">${content}</p>`;
    if (n.kind === 'input' && ['textarea','json-editor','markdown-editor'].includes(n.control?.kind)) return `<label class="dt-wire-input"><span>${label} · ${esc(n.control.kind)}</span><textarea readonly tabindex="-1" rows="4" aria-label="${label}" placeholder="${esc(n.text)}"></textarea></label>`;
    if (n.kind === 'input' && n.control?.kind==='select') return `<label class="dt-wire-input"><span>${label}</span><select disabled aria-label="${label}">${(n.control.options || []).map(o=>`<option>${esc(o.label)}</option>`).join('')}</select></label>`;
    if (n.kind === 'input') return `<label class="dt-wire-input"><span>${label}</span><input type="${n.control?.kind==='json-file'?'file':esc(n.control?.kind || 'text')}" disabled tabindex="-1" placeholder="${esc(n.text)}" aria-label="${label}"></label>`;
    ...
    if (n.kind === 'slot' && Object.hasOwn(fills,n.label)) { const fill=fills[n.label]; return `<div class="dt-wire-slot"><small>Slot: ${label}</small>${dtPreviewNodes(fill.doc,null,stack,stateName,budget,{},fill.ids)}</div>`; }
    ...
    const assignments=Object.fromEntries(Object.entries(n.slots || {}).map(([name,ids])=>[name,{doc,ids}]));
    const rows = dtPropRows(n), props = Object.fromEntries(rows.filter(r => r.value !== undefined).map(r => [r.name, r.value]));
    return `<section class="dt-wire-component"><div class="dt-wire-component-title"><strong>${esc(c?.name || n.component.label)}</strong><small>v${esc(n.component.version)} · ${esc(n.component.variantId)}</small></div>${nested ? dtPreviewNodes(detail, null, [...stack, n.component.id], stateName, budget, assignments) : `<p class="small muted">${esc(!c ? 'Definition missing — reference retained.' : c.version !== n.component.version ? 'Contract version changed — review the instance before previewing current internals.' : 'Reusable component wireframe. Design its internals in the component editor.')}</p>`}${!detail&&c&&c.version===n.component.version?Object.values(assignments).map(fill=>dtPreviewNodes(fill.doc,null,stack,stateName,budget,{},fill.ids)).join(''):''}${Object.keys(props).length ? `<dl class="dt-wire-props">${Object.entries(props).map(([key, value]) => `<dt>${esc(key)}</dt><dd>${esc(String(value))} <small class="muted">${esc(rows.find(r => r.name === key)?.source)}</small></dd>`).join('')}</dl>` : ''}</section>`;
  }).join('');
}
