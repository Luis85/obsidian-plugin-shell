...
const dsUi={selected:null,query:'',kind:'all',show:true,form:null,error:'',drag:null};
...
function dsOpen(kind,record={}){
 if(state.activeRun)return notify('Finish or cancel the active simulation before changing data sources.');
 dsUi.form={...designCopy(record),formKind:kind,owner:designOwner(),revision:design().revision,snapshot:dsToken()};dsUi.error='';showModal('data-source-form');
}
...
function dsEditSource(id){dsOpen('source',dataSources().sources.find(s=>s.id===id)||dsFreshSource());}
...
function dsEditFlow(id=null,sourceId=null,cardId=null,direction=null){
 const m=dataSources(),sources=m.sources.filter(s=>s.status!=='deprecated'&&s.operations.length),cards=design().nodes.filter(n=>n.kind!=='group');
 const old=m.flows.find(f=>f.id===id);if(!old&&(!sources.length||!cards.length))return notify('Add a source operation and a sitemap surface before connecting data.');
 const source=sources.find(s=>s.id===sourceId)||sources[0],op=source?.operations[0];
 dsOpen('flow',old||{id:null,source:source.id,operation:op.id,card:cardId||selectedNode()?.id||cards[0].id,direction:direction&&(op.direction===direction||op.direction==='both')?direction:op.direction,label:op.name,trigger:'manual',notes:''});
}
...
function dsSave(){
 const f=dsUi.form;if(!f||f.owner!==designOwner()||f.revision!==design().revision||f.snapshot!==dsToken())return dsFail('This review is stale. Copy your edits and reopen it; nothing was overwritten.');
 let selected=dsUi.selected;
 const ok=dsCommit((m,d)=>{
  if(f.formKind==='source'){
   ...
   dsPosition(source.id,m,d);if(old)m.flows[m.flows.indexOf(old)]=flow;else m.flows.push(flow);selected=source.id;
 ...
 if(ok){dsUi.selected=selected;modalOriginal=null;closeModal();render();notify(f.formKind==='position'?'Source positioned. Generator contracts unchanged.':'Data-source design saved. No external service, database or vault was accessed.');}return ok;
}
function handleDataSourceAction(action,value=''){
 // Source nodes are a separate projection, never native view/containment nodes.
 if(action==='canvas-edge'&&dataSources().flows.some(f=>f.id===value)){canvasUi.edge=value;designUi.selected=dataSources().flows.find(f=>f.id===value).card;paintMapSelection();dsEditFlow(value);return true;}
 ...
 switch(action){
  case 'ds-example':if(dsCommit(seedDataSourceExample)){dsUi.selected=dataSources().sources[0]?.id;render();}break;
  case 'ds-add':dsOpen('source',dsFreshSource(Object.hasOwn(DS_KINDS,value)?value:'api'));break;
  case 'ds-edit':dsEditSource(value||dsUi.selected);break;
  case 'ds-select':dsUi.selected=value;render();break;
  case 'ds-save':dsSave();break;
  case 'ds-operation':dsEditOperation(value);break;
  case 'ds-connect':dsEditFlow(null,value||dsUi.selected);break;
  case 'ds-card-connect':dsEditFlow(null,null,value);break;
  case 'ds-flow':dsEditFlow(value);break;
  case 'ds-remove-source':{const s=dataSources().sources.find(s=>s.id===value);if(s)dsOpen('remove-source',s);break;}
  case 'ds-remove-operation':{const [sourceId,id]=value.split(':');dsOpen('remove-operation',{id,sourceId});break;}
  case 'ds-remove-flow':dsOpen('remove-flow',{id:value});break;
  case 'ds-place':if(dsCommit((m,d)=>dsPosition(value,m,d))){dsUi.show=true;dsUi.selected=value;designUi.selected=null;setView('sitemap');dsPick(value);fitMap();}break;
  ...
  case 'ds-catalog':dsUi.selected=value||dsUi.selected;setView('sources');break;
...
function editDataSourceField(el){
 ...
 if(key==='ds-query'||key==='ds-filter'){dsUi[key==='ds-query'?'query':'kind']=el.value;const results=document.getElementById('ds-results');if(results)results.innerHTML=dsCatalogItems();return true;}
 ...
 if(key.startsWith('ds-shape-')){
  ...
  if(name==='mode'){if(!f.shapeDrafts)f.shapeDrafts={input:{},output:{}};f.shapeDrafts[side][s.mode]=designCopy(s);f[side]=designCopy(f.shapeDrafts[side][el.value]||dsNewShape(el.value));if(el.value==='entity'&&!f[side].entity)f[side].entity=design().semantic?.entities[0]?.id||null;redrawModal();}
 ...
 const name=key.slice(3),previous=f.name;if(['name','slug','kind','status','description','locator','auth','credentialRef','direction','method','resource','source','operation','card','label','trigger','notes','x','y'].includes(name))f[name]=el.value;
 ...
 if(name==='source'){const o=dataSources().sources.find(s=>s.id===f.source)?.operations[0];f.operation=o?.id||'';f.direction=o?.direction||'read';f.label=o?.name||'';redrawModal();}
 if(name==='operation'){const o=dataSources().sources.find(s=>s.id===f.source)?.operations.find(o=>o.id===f.operation);f.direction=o?.direction||'read';redrawModal();}
