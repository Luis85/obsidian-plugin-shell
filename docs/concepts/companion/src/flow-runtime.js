...
function flowNodeContents(n,lib,children,collapsed){
 ...
 const bindings=(n.components||[]).map(b=>{const comp=lib.find(x=>x.id===b.id);return `<button class="flow-binding-chip nodrag nopan" data-action="flow-binding" data-value="${esc(n.id+':'+b.id+':'+b.slot)}" title="Edit ${esc(comp?.name||b.id)} placement in ${esc(b.slot)}">${esc(comp?.name||b.id)}<span>${esc(b.slot)}</span></button>`;}).join('');
...
function createFlowNodeComponent(){
 const {h,defineComponent}=Vue;const {Handle,Position}=VueFlowCore;
 return defineComponent({name:'CompanionSurfaceCard',props:['id','data','selected'],setup(props){return ()=>{
...
function mountFlow(){
 ...
 const owner=designOwner(),serial=++flowUi.serial;flowUi.owner=owner;flowUi.cancelled=false;flowUi.dragging=false;flowUi.connecting=false;
 ...
 const Component=defineComponent({name:'CompanionSitemap',setup(){
  ...
  api.onNodesChange(changes=>{if(current())api.applyNodeChanges(changes.filter(x=>['dimensions','position','select'].includes(x.type)));});
  api.onEdgesChange(changes=>{if(current())api.applyEdgeChanges(changes.filter(x=>x.type==='select'));});
  api.onInit(()=>{nextTick(()=>{if(current())paintFlowViewport();});});
  api.onNodesInitialized(()=>{nextTick(()=>{if(current())paintFlowViewport();});});
  ...
  api.onNodeDragStop(({node,event})=>{
   ...
   if(flowUi.cancelled||drag.revision!==design().revision){design().canvas=drag.before;flowUi.cancelled=false;render();canvasAnnounce('Movement cancelled. Previous position restored.');return;}
   const pos={x:Math.max(-50000,Math.min(50000,node.position.x)),y:Math.max(-50000,Math.min(50000,node.position.y))};
   if(JSON.stringify(pos)!==JSON.stringify(drag.before.positions[node.id])){recordDesign();canvasState().positions[node.id]=pos;canvasState().custom=true;save();}
   render();canvasAnnounce('Card moved. One undo restores its position; structure is unchanged.');focusMapNode(node.id);
  });
  api.onConnectStart(()=>{if(current()){flowUi.cancelled=false;flowUi.connecting=true;}});
  api.onClickConnectStart(()=>{if(current()){flowUi.cancelled=false;flowUi.connecting=true;}});
  api.onConnect(connection=>{if(current()&&!flowUi.cancelled){flowUi.connecting=false;connectionUi.committed=true;reviewFlowConnection(connection);}});
  api.onConnectEnd(()=>{if(current())flowUi.connecting=false;});
  api.onClickConnectEnd(()=>{if(current())flowUi.connecting=false;});
  ...
  return ()=>h(VueFlow,{id:'companion-map-'+serial,nodes:store.nodes,edges:store.edges,nodeTypes,edgeTypes,applyDefault:false,connectionMode:ConnectionMode.Strict,connectOnClick:false,isValidConnection:validFlowConnection,panOnScroll:store.preferences.wheel==='pan',zoomOnScroll:store.preferences.wheel==='zoom',zoomOnPinch:store.preferences.pinch,zoomOnDoubleClick:false,zoomActivationKeyCode:['Control','Meta'],panOnDrag:store.preferences.panOnDrag,preventScrolling:store.preferences.wheel!=='page',minZoom:.12,maxZoom:2,snapToGrid:store.snap,snapGrid:[24,24],nodeDragThreshold:0,paneClickDistance:5,deleteKeyCode:null,selectionKeyCode:null,multiSelectionKeyCode:null,nodesFocusable:false,edgesFocusable:true,defaultViewport:{x:canvasState().pan.x,y:canvasState().pan.y,zoom:canvasState().zoom},noDragClassName:'nodrag',noPanClassName:'nopan',noWheelClassName:'nowheel'});
 }});
...
function destroyFlow(){
 ...
 const app=flowUi.app,store=flowUi.store,pinia=flowUi.pinia;flowUi.app=null;flowUi.api=null;flowUi.store=null;flowUi.pinia=null;flowUi.drag=null;flowUi.dragging=false;flowUi.connecting=false;
 app.unmount();store?.$dispose();if(Pinia.disposePinia)Pinia.disposePinia(pinia);
}
...
function cancelFlowGesture(){
 ...
 if(flowUi.connecting){flowUi.cancelled=true;flowUi.connecting=false;flowUi.api?.endConnection();flowUi.api?.endConnection(undefined,true);canvasAnnounce('Connection cancelled. Nothing changed.');return true;}
 return false;
}
