...
function flowNodeProjection(d){const c=canvasState(d);return [...dataSourceNodeProjection(d),...visibleMapNodes(d).map(n=>({id:n.id,type:'surface',position:{...c.positions[n.id]},data:{surface:designCopy(n),library:designCopy(d.library),dim:!mapMatch(n),collapsed:c.collapsed.includes(n.id),childCount:d.nodes.filter(x=>x.parent===n.id).length},selected:n.id===designUi.selected,draggable:!state.activeRun,selectable:true,connectable:true,focusable:false,dimensions:brickSurfaceSize(n,d)}))];}
...
function createFlowNodeComponent(){
 ...
 return defineComponent({name:'CompanionSurfaceCard',props:['id','data','selected'],setup(props){const serial=flowUi.serial;onMounted(()=>scheduleHandleMeasure(props.id,serial));watch(()=>JSON.stringify([props.data.surface.kind,props.data.surface.layout,props.data.surface.bricks]),()=>scheduleHandleMeasure(props.id,serial),{flush:'post'});return ()=>{
  ...
  return h('article',{class:['map-node','kind-'+n.kind,{'selected':props.selected,'search-dim':props.data.dim,'brick-page':brickDisplay()!=='structure'&&canHaveBricks(n)&&n.kind!=='view'}],style:{'--brick-height':brickSurfaceSize(n).height+'px',width:brickSurfaceSize(n).width+'px',height:brickSurfaceSize(n).height+'px'},tabindex:0,'data-node':n.id,'data-action':'canvas-select','data-value':n.id,'aria-label':n.label+' · '+NODE_KINDS[n.kind],role:'group','aria-current':props.selected?'true':undefined},[h('div',{class:'flow-node-content',innerHTML:flowNodeContents(n,props.data.library,props.data.childCount,props.data.collapsed)}),...handles,...(n.kind!=='group'?dsPorts(n.id,false):[])]);
 };}});
}
function mountFlow(){
 ...
 const Component=defineComponent({name:'CompanionSitemap',setup(){
  ...
  const nodeTypes={surface:markRaw(createFlowNodeComponent()),dataSource:markRaw(createDataSourceNode())};const edgeTypes={transition:markRaw(createFlowEdgeComponent()),structure:markRaw(createFlowEdgeComponent())};
  api.onError(error=>{if(current())console.error('[Sitemap] '+error.message);});
  api.onNodesChange(changes=>{if(current())api.applyNodeChanges(dsAlignChanges(alignFlowChanges(changes.filter(x=>['dimensions','position','select'].includes(x.type)))));});
  ...
  api.onEdgeUpdate(({edge,connection})=>{if(current())reviewEdgeReconnect(edge,connection);});
  api.onEdgeUpdateEnd(()=>{if(current())endEdgeReconnect();});
  api.onPaneClick(()=>{if(current()&&!flowUi.suppressClick&&!canvasUi.placing)deselectFlow();});
  api.onViewportChange(v=>{if(!current()||flowUi.syncing)return;const c=canvasState();c.pan=boundedCameraPan(v);c.zoom=Math.max(.12,Math.min(2,v.zoom));c.fitted=true;paintFlowChrome();clearTimeout(flowUi.saveTimer);flowUi.saveTimer=setTimeout(()=>{if(current())save();},150);});
  return ()=>h(VueFlow,{id:'companion-map-'+serial,nodes:store.nodes,edges:store.edges,nodeTypes,edgeTypes,applyDefault:false,connectionMode:ConnectionMode.Strict,edgeUpdaterRadius:14,connectOnClick:false,isValidConnection:validFlowConnection,panOnScroll:store.preferences.wheel==='pan',zoomOnScroll:store.preferences.wheel==='zoom',zoomOnPinch:store.preferences.pinch,zoomOnDoubleClick:false,zoomActivationKeyCode:['Control','Meta'],panOnDrag:store.preferences.panOnDrag,preventScrolling:store.preferences.wheel!=='page',minZoom:.12,maxZoom:2,snapToGrid:store.snap,snapGrid:[24,24],nodeDragThreshold:4,paneClickDistance:5,deleteKeyCode:null,selectionKeyCode:null,multiSelectionKeyCode:null,nodesFocusable:false,edgesFocusable:true,defaultViewport:{x:canvasState().pan.x,y:canvasState().pan.y,zoom:canvasState().zoom},noDragClassName:'nodrag',noPanClassName:'nopan',noWheelClassName:'nowheel'});
 }});
