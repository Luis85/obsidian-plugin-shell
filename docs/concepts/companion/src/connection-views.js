...
function structureRelationshipDialog(){
 ...
 ${connectionSelect('Parent container','structure-parent',[['','Plugin root'],...parents.map(n=>[n.id,n.label+' · '+NODE_KINDS[n.kind]])],f.parent||'')}
 ...
 <p class="connection-save-note">The line chooses the nearest sides as cards move. Layout is not locked bottom-to-top.</p><p class="error" id="connection-error" role="alert">${esc(connectionUi.error)}</p>`,button('Cancel','close','','ghost')+button('Save parent','connection-structure-save','','primary','check'));
}
