...
export async function makerFixture(work, { temporaryRoot = tmpdir() } = {}) {
  ...
  try {
    ...
    for (const file of [
      ...
      'events.ts',
      'event-catalog.ts',
      'core-event-catalog.ts',
      'example-events.ts',
      'example-event-catalog.ts',
    ])
      await cp(join(makerSourceRoot, 'src/bootstrap', file), join(root, 'src/bootstrap', file));
...
export async function installMakerFoundation(root) {
  const paths = [
    ...
    'src/features/api.ts',
    'src/features/showcase/events.ts',
...
export async function removeMakerExamples(root) {
  ...
  const source = (await readFile(path, 'utf8'))
    ...
    .filter(
      (line) =>
        !line.includes('../features/tasks/definition') &&
        !line.includes('../features/projects/definition') &&
        !/^\s*(task|project):/.test(line),
    )
    .join('\n');
