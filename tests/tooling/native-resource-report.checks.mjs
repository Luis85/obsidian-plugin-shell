...
import { nativeOwnershipFixture, nativeOwnershipObservation as observation, nativeOwnershipChecks } from './native-ownership-fixture.mjs';

const checks = nativeOwnershipChecks.basic;
const enhancedChecks = nativeOwnershipChecks.enhanced;
const closeCheck = checks[1]; const sibling = 'view:sibling:showcase-notice';
const fixture = () => nativeOwnershipFixture();
const enhancedFixture = () => nativeOwnershipFixture(true);
...
test('native ownership rejects transient resurrection of closed owners even when the final resource count returns to zero', () => {
  for (const owner of ['view:cancelled:recovery', 'view:first:recovery', 'view:first:showcase-notice']) {
    const value = enhancedFixture(); const resources = value.resourceOwnership;
    const cancelled = owner === 'view:cancelled:recovery';
    const offset = (cancelled ? resources.cancelledProgress : resources.closed).events.length;
    const acquired = { resource: 'notice', phase: 'acquired', owner, operation: 'progress', id: 900, count: 1 };
    const stages = cancelled ? ['opened', 'closed', 'replayed', 'unloaded', 'finalObservation'] : ['unloaded', 'finalObservation'];
    for (const stage of stages) {
      const entries = resources[stage].events.map(event => event.entry);
      entries.splice(offset, 0, acquired, { ...acquired, phase: 'released' });
      resources[stage] = observation(entries);
    }
    assert.throws(() => validateNativeOwnership(value, enhancedChecks), /EVIDENCE_NATIVE_OWNERSHIP/);
  }
});

test('native ownership rejects sibling or new-owner acquisition after plugin unload even if immediately released', () => {
  for (const owner of ['view:sibling:showcase-notice', 'view:new:unexpected']) {
    const value = enhancedFixture(); const final = value.resourceOwnership.finalObservation;
    const acquired = { resource: 'notice', phase: 'acquired', owner, operation: 'example', id: 900, count: 1 };
    value.resourceOwnership.finalObservation = observation([...final.events.map(event => event.entry), acquired, { ...acquired, phase: 'released' }]);
    assert.throws(() => validateNativeOwnership(value, enhancedChecks), /EVIDENCE_NATIVE_OWNERSHIP/);
  }
});
