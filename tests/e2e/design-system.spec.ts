...
async function open(page: Page) {
  ...
  if(process.env.GENERATED_STYLES==='1') {
    // The built generator output must style controls before any test-only theme is injected.
...
test('[DS-NUXT] compiled saved tokens style real Nuxt buttons and inputs without changing the host',async({page})=>{
  ...
  const primary=page.getByRole('button',{name:'Create your first Task note'});
  await expect(primary).toHaveCSS('background-color','rgb(101, 80, 185)');
  await expect(primary).toHaveCSS('border-radius','6px');
  await expect(primary).toHaveCSS('font-size','14px');
  await expect(page.getByRole('heading',{name:'Your plugin starts here.'})).toHaveCSS('font-size','24px');
  ...
  await input.focus();await expect(input).toHaveCSS('outline-color','rgb(101, 80, 185)');
...
test('[DS-MODE] host mode changes update a live Nuxt control and independent owned roots',async({page})=>{
  ...
  for(const root of await page.locator(scope).all())await expect(root).toHaveCSS('background-color','rgb(30, 30, 36)');
  await page.locator('#theme-light').click();
  await expect.poll(()=>primary.evaluate(e=>getComputedStyle(e).backgroundColor)).toBe('rgb(101, 80, 185)');
  await page.emulateMedia({forcedColors:'active'});
  await expect(primary).not.toHaveCSS('background-color','rgb(101, 80, 185)');
});
test('[DS-HOST] host palette follows host preferences while typography remains declared',async({page})=>{
  ...
  const primary=page.getByRole('button',{name:'Create your first Task note'});
  await expect(primary).toHaveCSS('background-color','rgb(18, 52, 86)');
  await expect(primary).toHaveCSS('font-size','14px');
