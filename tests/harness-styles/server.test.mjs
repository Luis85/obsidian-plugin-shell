...
test('[HTTP-01] serves an explicitly labeled stylesheet fixture', async () => {
  ...
  assert.match(result.body, /Extracted Obsidian stylesheet/);
...
test('[HTTP-03] serves all five ordered CSS modules', async () => {
  const entry = await get('/harness/styles/simulated.css');
...
test('[HTTP-07] source-level tripwires: fixture provenance, no external assets or HTML injection', async () => {
  const entry = await readFile(new URL('../../harness/styles/simulated.css', import.meta.url), 'utf8');
...
test('[HTTP-11] serves decoded pinned extraction and plugin aliases without exposing its archive', async () => {
  ...
  for (const path of ['harness/styles/host-adapter.css','src/styles/index.css','src/styles/tokens.css']) {
    const response = await get(`/${path}`); assert.equal(response.status,200);
    assert.equal(response.body,await readFile(new URL(`../../${path}`,import.meta.url),'utf8'));
  }
});
