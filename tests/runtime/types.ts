...
const authorFact: EventDefinition<'author.completed', { readonly value: number }> = defineEvent(
  'author.completed',
  (value: unknown): value is { readonly value: number } =>
    typeof value === 'object' && value !== null && 'value' in value && typeof value.value === 'number',
);
export function eventTypeContract(
  observer: EventObserver<EventMapOf<typeof authorFact>>,
  publisher: EventPublisher<EventMapOf<typeof authorFact>>,
  subscriber: EventSubscriber<EventPayload<typeof authorFact>>,
): void {
  observer.on('author.completed', (payload) => {
    void payload.value;
  });
  subscriber.once((payload) => {
    void payload.value;
  });
  publisher.publish({ type: 'author.completed', payload: { value: 1 } });
  // @ts-expect-error An observer cannot publish facts.
  observer.publish({ type: 'author.completed', payload: { value: 1 } });
  // @ts-expect-error A feature publisher cannot publish another owner's fact.
  publisher.publish({ type: 'host.metadata.changed', payload: { path: 'private' } });
  // @ts-expect-error A subscriber cannot acquire another event's publisher.
  subscriber.publisher(authorFact);
}
...
export function featureTypeContract(shared: Parameters<typeof createNoteFeatures>[0]): void {
  const features = createNoteFeatures(shared, (register) => ({
