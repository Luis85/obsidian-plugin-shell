...
try:
    with sync_playwright() as pw:
        ...
        p.reload()
        check('Native browser storage restores committed project data after reload', p.evaluate('!!state.activeId&&design().goal==="Real-origin saved outline"&&!storageWarning'))
        ...
        check('Real storage event stops the stale window from overwriting newer data', p.evaluate('JSON.parse(localStorage.getItem(STORAGE_KEY)).projects[0].design.goal==="Newer second-window edit"'))
