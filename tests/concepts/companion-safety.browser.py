...
with sync_playwright() as pw:
    ...
    try:
        ...
        check('A complete saved snapshot restores with valid ownership and content', q.evaluate('!!state.project&&designIssues(design()).every(i=>i.level!=="error")&&!storageWarning'), 'controlled storage reload fixture')
        q.close()
        ...
        act(p, 'reset-confirm', scope='#modal')
        check('Explicit successful reset returns to fresh state', p.evaluate('state.project===null&&!storageWarning&&JSON.parse(__storage.value).project===null'))
        p.close()
        p = new(width=390)
        ...
        p.close()
        p = new()
        p.evaluate('window.__retained=__storage.value;state.project.notes=["x".repeat(5000000)];save()')
