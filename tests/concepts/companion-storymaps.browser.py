...
with sync_playwright() as pw:
    ...
    try:
        ...
        act('sm-select', story_id); act('sm-move', story_id, '#sm-inspector'); history = js('design().history.length')
        ...
        check('Keyboard-submitted non-drag move updates step and release in one history entry', js('smCurrentMap().stories[0].stepId') == second_step and js('smCurrentMap().stories[0].releaseId') == release_id and js('design().history.length') == history + 1)
        ...
        history = js('design().history.length'); page.mouse.move(coords['x'], coords['y']); page.mouse.down(); page.mouse.move(coords['tx'], coords['ty'], steps=18)
        ...
        check('Actual pointer drop commits the intended cell, not coordinates', js('smCurrentMap().stories[0].stepId') == s_id and js('smCurrentMap().stories[0].releaseId') is None and js('design().history.length') == history + 1)
