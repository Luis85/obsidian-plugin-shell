...
with sync_playwright() as pw:
 ...
 try:
  ...
  check('Complete self-project imports through review and explicit confirmation',js('validState(state)&&dtStore().documents.length===80&&dtStore().revisions.length===54'))
  ...
  act('nav','pages','#sidebar');check('Pages reports complete page and component design coverage','26/26 page designs' in page.locator('#content').inner_text() and '54/54 component designs' in page.locator('#content').inner_text())
  ...
  act('dt-undo');check('Undo remains available on the complete 80-design project',js('dtDocument().nodes[0].ui.gap!==24&&dtStore().revisions.length===54'))
  ...
  for item in inventory:
   ...
   check('Rendered design '+item['id'],page.locator('.dt-preview-shell').is_visible() and page.locator('.dt-wireframe [data-design-node]').count()>0,'Direct canonical route selection and actual rendered DOM, not 80 independent end-user journeys')
