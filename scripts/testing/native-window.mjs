...
export async function sizeNativeWindow(page) {
  ...
  if (process.platform === 'linux') {
    ...
    await expect.poll(() => {
      const ids = probe(['search', '--onlyvisible', '--maxdepth', '1', '--name', '.*']).split(/\s+/).filter(id => /^\d+$/.test(id));
      inventory = ids.map(id => ({ id, title: probe(['getwindowname', id]) }));
      const exact = inventory.filter(item => title && item.title === title);
      candidates = exact.length ? exact : inventory.filter(item => /obsidian/i.test(item.title));
      writeFileSync('reports/native/window-inventory.json', JSON.stringify({ documentTitle: title, inventory, candidates }, null, 2));
