...
export interface DetailElement {
  ...
  a11y: string; visibleIn: DetailState[];
}
export interface DetailInteraction {
  id: string; source: string; target: string; event: string; label: string;
  notes: string; acceptance: string; targetSurfaceId: string | null; effect?: {type:'state'|'toggle'|'value'|'focus'|'emit';value: DetailLiteral;payload?:DetailLiteral};
}
...
export interface DetailRequest {
  ...
  event: string; values: Readonly<Record<string, unknown>>; payload: unknown;
}
