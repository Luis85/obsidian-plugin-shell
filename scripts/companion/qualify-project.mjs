...
const output = join(root, 'reports/project-generator'); await mkdir(output, { recursive: true });
const vault = await mkdtemp(join(process.env.RUNNER_TEMP ?? tmpdir(), 'companion-qualification-'));
const options = { input: join(root, 'docs/concepts/companion/companion-project.json'), vault, target: 'companion' };
