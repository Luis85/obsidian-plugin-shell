...
function element(doc: DetailDocument, node: DetailElement, componentNames: Map<string, string>): string {
  ...
  if (node.kind === 'divider') return `<hr ${attrs} ${events} />`;
  if (node.kind === 'image') return `<div ${attrs} role="img" :aria-label="${ref}.label" class="generated-asset" ${events}>{{ ${ref}.label }} — {{ ${ref}.text }}</div>`;
  if (['input','number','textarea','checkbox','select'].includes(node.kind)) {
    const common=`:id="model.prefix + '-${node.id}'" :aria-describedby="${ref}.a11y ? model.prefix + '-${node.id}-hint' : undefined" ${disabled} @input="model.update(${index}, $event)" ${events}`;
    const control=node.kind==='textarea'?`<textarea ${common} :value="model.display(${index})" :placeholder="${ref}.text" />` : node.kind==='select'?`<select ${common} :value="model.display(${index})"><option v-for="option in ${ref}.options" :key="option" :value="option">{{option}}</option></select>` : `<input ${common} type="${node.kind==='input'?'text':node.kind}" ${node.kind==='checkbox'?`:checked="model.read(${index}) === true"`:`:value="model.display(${index})" :placeholder="${ref}.text"`} />`;
    return `<div ${attrs} class="generated-field"><label :for="model.prefix + '-${node.id}'">{{ ${ref}.label }}</label>${control}<small v-if="${ref}.a11y" :id="model.prefix + '-${node.id}-hint'">{{ ${ref}.a11y }}</small></div>`;
  }
  if (node.kind === 'button') return `<button ${attrs} type="button" ${disabled} ${events}>{{ model.display(${index}) || ${ref}.label }}</button>`;
  if (node.kind === 'tabs') return `<div ${attrs} ${events}><div role="tablist" :aria-label="${ref}.label"><button v-for="(option,i) in ${ref}.options" :key="option" role="tab" type="button" :aria-selected="option === model.display(${index}) || (!model.values[${ref}.id] && i===0)" ${disabled} @click="model.tab(${index},option)">{{option}}</button></div></div>`;
  if (node.kind === 'table') return `<div ${attrs} class="generated-table" ${events}><table><caption>{{${ref}.label}}</caption><thead><tr><th v-for="column in ${ref}.options" :key="column" scope="col">{{column}}</th></tr></thead><tbody><tr v-for="(row,i) in model.rows(${index})" :key="i"><td v-for="column in ${ref}.options" :key="column">{{model.cell(row,column)}}</td></tr><tr v-if="!model.rows(${index}).length"><td :colspan="${ref}.options?.length || 1">{{${ref}.text}}</td></tr></tbody></table></div>`;
  if (node.kind === 'list') return `<div ${attrs} ${events}><strong>{{${ref}.label}}</strong><ul><li v-for="(item,i) in (model.rows(${index}).length ? model.rows(${index}) : ${ref}.options || [])" :key="i">{{item}}</li></ul></div>`;
  if (node.kind === 'slot') return `<div ${attrs} ${events}><slot :name="${ref}.label">${children || `{{ ${ref}.text }}`}</slot></div>`;
  const slots=[...new Set(doc.nodes.filter(n=>n.parentId===node.id).map(n=>n.slotName!))].map(name=>`<template #${name}>${doc.nodes.filter(n=>n.parentId===node.id && n.slotName===name).map(n=>element(doc,n,componentNames)).join('\n')}</template>`).join('\n');
...
const model = useDetail(spec, props, request => emit('interaction', request), (name,payload) => { ${doc.kind==='component' ? Object.entries(componentMembers(detailDefinitions(m).find(c=>c.id===doc.ownerId)!).events).map(([event,type])=>`if(name===${literal(event)}) emit(${literal(event)}, ${type==='undefined'?'undefined':'payload as '+type});`).join(' ') : 'void name; void payload;'} });
</script>
<template>
...
`);
  ...
  add(`${root}/presentation/composables/use-detail.ts`, (await readFile(join(templateRoot, 'scripts/companion/runtime/use-detail.ts'), 'utf8')).replace("'./detail-runtime.ts'", "'../../domain/detail-runtime.ts'").replace("'../composition-contract.mjs'","'../../domain/composition-contract.mjs'"));
...
`);
    ...
    for (const edge of doc.edges) {
      ...
      if (!edge.targetSurfaceId && !edge.effect) {
      ...
      trace.push({ documentId: doc.id, ...edge, component: path, implementation: edge.effect || edge.targetSurfaceId ? null : implementation, test: !edge.effect && (edge.acceptance || !edge.targetSurfaceId) ? test : null, verification: edge.effect ? 'executable-ui-effect' : edge.targetSurfaceId ? 'navigation-scaffold' : 'todo' });
    }
