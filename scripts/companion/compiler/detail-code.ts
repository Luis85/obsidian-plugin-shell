...
function element(doc: DetailDocument, node: DetailElement, componentNames: Map<string, string>): string {
  ...
  if (node.kind === 'input') return controlElement(node, index, attrs, disabled);
  ...
  const slots = Object.entries(node.slots ?? {}).map(([name, ids]) => `<template #${name}>${ids.map(id => element(doc, doc.nodes.find(n => n.id === id)!, componentNames)).join('\n')}</template>`).join('\n');
...
function documentCode(m: Model, doc: DetailDocument): string {
  ...
  const events = doc.kind === 'component' ? componentMembers(m.components.find(c => c.id === doc.ownerId)!).events : {};
...
const model = useDetail(spec, props, request => emit('interaction', request), (event, payload) => { switch(event) { ${emitted} default: throw new Error('DETAIL_EMIT_UNKNOWN'); } });
</script>
<template>
...
`);
  ...
  add(`${root}/presentation/composables/use-detail.ts`, (await readFile(join(templateRoot, 'scripts/companion/runtime/use-detail.ts'), 'utf8')).replaceAll("'./detail-runtime.ts'", "'../../domain/detail-runtime.ts'").replaceAll("'./detail-controls.ts'", "'../../domain/detail-controls.ts'").replaceAll("'./detail-actions.ts'", "'../../domain/detail-actions.ts'"));
...
`);
    ...
    for (const edge of doc.edges) {
      ...
      if (!edge.targetSurfaceId && !edge.action) {
      ...
      trace.push({ documentId: doc.id, ...edge, component: path, implementation: edge.targetSurfaceId || edge.action ? null : implementation, test: edge.acceptance || !edge.targetSurfaceId ? test : null, verification: edge.action ? 'declarative-action' : edge.targetSurfaceId ? 'navigation-scaffold' : 'todo' });
    }
