...
export async function projectFiles(templateRoot: string, m: Model): Promise<Entry[]> {
  ...
  dataCode(m,add); navigationCode(m,add); uiCode(m,add); hostCode(m,add); await detailCode(templateRoot,m,add);
