...
export async function projectFiles(templateRoot: string, m: Model): Promise<Entry[]> {
  ...
  add('design/traceability.json',json({status:'scaffold-not-accepted',requirements:m.requirements.map(r => ({...r,implementation:`${m.sourceRoot}/application/use-cases/${r.key}.ts`,test:`${m.testRoot}/acceptance/${r.key}.test.ts`,verification:'todo'})),interactions:m.links,flows:m.flows,warnings:m.warnings}),'managed');
  ...
  dataCode(m,add); navigationCode(m,add); uiCode(m,add); hostCode(m,add);
