...
export async function projectFiles(templateRoot: string, m: Model): Promise<Entry[]> {
  ...
  pkg.scripts['verify:project'] = 'npm run build && npm run typecheck:project && npm test';
  ...
  dataCode(m,add); navigationCode(m,add); uiCode(m,add); hostCode(m,add); styleCode(m,add); await persistenceCode(templateRoot,m,add); await detailCode(templateRoot,m,add);
