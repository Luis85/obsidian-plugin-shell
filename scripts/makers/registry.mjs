...
export async function readRegistry(root) {
  ...
  const path = 'src/bootstrap/features.ts'; const source = await readFile(resolve(root, path), 'utf8');
  ...
  const registrations = []; let object; let registerName;
  function visit(node) {
    if (ts.isCallExpression(node) && ts.isIdentifier(node.expression) && node.expression.text === 'createNoteFeatures') {
      ...
      if (object || !literal || !ts.isObjectLiteralExpression(literal) || callback.parameters.length !== 1 || !ts.isIdentifier(callback.parameters[0].name)) throw new Error('REGISTRY_UNSUPPORTED_SHAPE: use one explicit register callback object');
      object = literal; registerName = callback.parameters[0].name.text;
    }
  ...
  return { path, source, imports, registrations, importEnd: lastImport.end, close: object.end - 1, lastPropertyEnd: object.properties.at(-1)?.end, trailingComma: object.properties.hasTrailingComma };
}
export function extendRegistry(registry, { key, local, from }) {
  ...
  if (!/^\s*$/.test(registry.source.slice(lineStart, registry.close))) throw new Error('REGISTRY_UNSUPPORTED_LAYOUT: closing callback brace needs its own line');
  let source = registry.source.slice(0, lineStart) + `    ${key}: register(${local}),${newline}` + registry.source.slice(lineStart);
