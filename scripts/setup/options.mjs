...
const switches = new Set(['yes', 'no-interaction', 'dry-run', 'skip-install', 'no-local', 'help', 'json', 'resume', 'provision-browser', 'defer-verify']);
...
export async function setupOptions(args) {
  ...
  // The native install stage copies the artifacts that verification builds.
...
export function savedOptions(options) {
  return Object.fromEntries([...identityKeys, 'profile', 'migrate-from', 'skip-install', 'provision-browser', 'defer-verify'].filter(key => options[key] !== undefined).map(key => [key, options[key]]));
}
export async function resumeOptions(current, saved) {
  const args = [];
  for (const [key, value] of Object.entries(saved)) {
    if (![...identityKeys, 'profile', 'migrate-from', 'skip-install', 'provision-browser', 'defer-verify'].includes(key)) throw new Error('Unknown saved setup option');
