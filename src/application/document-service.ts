...
interface DocumentProjection { readonly title: string; readonly properties: Frontmatter; readonly body: string; readonly schemaVersion?: number }
export interface DocumentDefinition<I> {
  project(input: I): Result<DocumentProjection>;
}
...
export class DocumentCreationService<Inputs> {
  ...
  private prepareRequest<K extends keyof Inputs & string>(entity: K, values: Inputs[K], folderInput: string, requestId: string): Result<PreparedDocument> {
    ...
    const projection = this.project(entity, values);
    ...
    const prepared = this.materialize(entity, projection.value, folder.value, requestId);
    if (prepared.ok) this.requests.set(requestId, { input: fingerprint, plan: prepared.value });
    return prepared;
  }
  private project<K extends keyof Inputs & string>(entity: K, values: Inputs[K]): Result<DocumentProjection> {
    const definition = Object.hasOwn(this.definitions, entity) ? this.definitions[entity] : undefined;
    return definition ? definition.project(values) : failure('validation', 'error.entity');
  }
  private materialize(entity: string, projection: DocumentProjection, folder: string, requestId: string): Result<PreparedDocument> {
    const id = this.newId();
    if (!/^[a-zA-Z0-9-]{1,80}$/.test(id)) return failure('unexpected', 'error.unexpected');
    const { title, properties, body } = projection;
    const schemaVersion = projection.schemaVersion ?? 1;
